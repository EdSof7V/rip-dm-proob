...
const Profile = () => {
  ...
  const [dashboards, setDashboards] = useState<{ dashboardId: string; name: string }[]>([]);
    ...
    const fetchUserData = async () => {
      try {
        ...
        const allDashboards = await Promise.all(
          userGroups.map(groupId => getDashboardsByGroup(groupId))
        );
        setDashboards(allDashboards.flat());
      } catch (error) {
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
              <div className="relative drop-shadow-2">
                <Image
                  src={session?.user?.image || "/images/user/default-user.jpg"}
                  width={160}
                  height={160}
                  style={{
                    width: "auto",
            ...
            <div className="mt-4">
              ...
              <p className="font-medium">{session?.user?.roles || "Sin rol"}</p>
              ...
              <div className="mx-auto mb-5.5 mt-4.5 grid grid-cols-3 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F]">
                <h2 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Grupos:</h2>
                <ul className="max-w-md space-y-1 text-gray-500 list-inside dark:text-gray-400">
                  {groups.length > 0 ? groups.map(group => (
                    <li className="flex items-center" key={group}>
              ...
              <div className="mx-auto mb-5.5 mt-4.5 grid grid-cols-3 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F]">
                <h2 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Dashboards:</h2>
                <ul className="max-w-md space-y-1 text-gray-500 list-inside dark:text-gray-400">
                  {dashboards.length > 0 ? dashboards.map(d => (
                    <li className="flex items-center" key={d.dashboardId}>
                      <svg className="w-3.5 h-3.5 me-2 text-green-500 dark:text-green-400 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z" />
                      </svg>
                      {d.name || "Sin nombre"}
                    </li>
                  )) : <p>No hay dashboards asignados</p>}
                </ul>
              </div>
